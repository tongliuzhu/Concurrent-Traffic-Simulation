...
void createTrafficObjects_Paris(std::vector<std::shared_ptr<Street>> &streets, std::vector<std::shared_ptr<Intersection>> &intersections, std::vector<std::shared_ptr<Vehicle>> &vehicles, std::string &filename, int nVehicles)
{
    ...
    constexpr int nIntersections = 9;
    ...
    intersections.at(7)->setPosition(400, 1100);
     intersections.at(nIntersections-1)->setPosition(1700, 900); // central plaza
    ...
    for (size_t ns = 0; ns < nStreets; ns++)
    {
        ...
        streets.at(ns)->setOutIntersection(intersections.at(nIntersections-1));
    }
    ...
    for (size_t nv = 0; nv < nVehicles; nv++)
    {
        ...
        vehicles.at(nv)->setCurrentDestination(intersections.at(nIntersections-1));
    }
}
...
int main()
{
    ...
    int nVehicles = 6; // this number should be smaller than the amount of the street
